...
- [Kubernetes, 12-Factor & CI/CD (conceptual)](#kubernetes-12-factor--cicd-conceptual)
...
## Kubernetes, 12-Factor & CI/CD (conceptual)

These were intentionally not implemented, but here is how they would be applied.

### 12-Factor conformance

- **III. Config** — all environment-specific values (profile, datasource, port) come from environment variables; no secrets are hard-coded.
- **X. Dev/prod parity** — the same JPA code runs on H2 and MySQL; only config changes between environments.
- **VI. Processes** — the app is stateless; all state lives in the database.
- **V. Build/release/run** — Maven builds an immutable jar; the Docker image is the release artifact; runtime config is injected separately.
- **VII. Port binding** — the service self-hosts over a configurable HTTP port.
- **XI. Logs** — logs are written to stdout for the platform to collect.

### Kubernetes

The container would be deployed as a `Deployment` (multiple replicas for availability) fronted by a `Service`, with:

- a `ConfigMap` for non-sensitive config (profile, port) and a `Secret` for datasource credentials, both injected as environment variables;
- liveness/readiness probes backed by Spring Boot Actuator health endpoints;
- an external managed MySQL (or a StatefulSet) rather than the in-memory H2;
- an `Ingress` for external routing and horizontal scaling via an `HorizontalPodAutoscaler`.

### CI/CD

A pipeline would, on each push/PR: run `mvn clean verify` (tests + JaCoCo), enforce a coverage gate, build and scan the Docker image, publish it to a registry on merge to the main branch, and deploy to environments via the Kubernetes manifests above — promoting the same immutable image through stages.
